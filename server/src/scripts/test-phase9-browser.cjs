...
async function runBrowserVerification() {
  ...
  try {
    ...
    await context1.overridePermissions('http://localhost:5173', ['camera', 'microphone']);
    const context2 = browser2.defaultBrowserContext();
    await context2.overridePermissions('http://localhost:5173', ['camera', 'microphone']);
    ...
    pageTara.on('console', msg => {
      const txt = msg.text();
      if (txt.includes('call') || txt.includes('socket') || txt.includes('RTC') || txt.includes('error')) {
        console.log('  [Tara]', txt);
      }
    });
    ...
    // Ensure conversation is selected if needed
    await sleep(2500);

    // [2/7] Tara initiates video call
    console.log('\n[2/7] Tara initiating video call...');
    const videoCallBtn = await pageTara.$('button[title="Video call"]');
    if (!videoCallBtn) {
    ...
    try {
      ...
      await pageTara.screenshot({ path: path.join(ARTIFACT_DIR, 'debug_tara_call.png') });
      await pageAdi.screenshot({ path: path.join(ARTIFACT_DIR, 'debug_adi_call.png') });
      throw e;
    }
